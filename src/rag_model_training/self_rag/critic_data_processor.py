...
def create_utility_data(input_data):
    ...
    print(f"Utility distribution: {Counter([item['output'] for item in processed_data])}")
...
def create_retrieval_data_input_only(input_data):
    ...
    print(f"Input-only retrieval data sample: {processed_data[-1] if processed_data else 'None'}")
    print(f"Total input-only retrieval data: {len(processed_data)}")
    print(f"Input-only retrieval distribution: {Counter([item['output'] for item in processed_data])}")
    return processed_data
...
def create_groundness_data(input_data):
    ...
    print(f"Groundness distribution: {Counter([item['output'] for item in processed_data])}")
    return processed_data
...
def create_relevance_data(input_data, relevant_sampling_rate=0.7):
    ...
    print(f"Relevance distribution: {Counter([item['output'] for item in processed_data])}")
    return processed_data
...
def main():
    ...
    if relevance_data:
        final_train_data += create_relevance_data(relevance_data, config.get("relevant_sampling_rate", 0.7))
    ...
    final_train_data = [
        ...
        if "### Response:" not in item["input"] and "### Response:" not in item["instruction"]
    ]
