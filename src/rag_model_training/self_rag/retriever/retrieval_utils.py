...
class Retriever:
    ...
    def index_encoded_data(self, index, embedding_files, indexing_batch_size):
        ...
        for _i, file_path in enumerate(embedding_files):
...
def create_retrieval_input(
    ...
    output_file: str,
    need_retrieval_files: list[str] | None = None,
) -> None:
    ...
    if need_retrieval_files:
        ...
        combined_data = [item for item in combined_data if str(item.get("id", "")) in need_retrieval]
...
def process_retrieval_results(input_file: str, output_file: str, top_k: int = 5) -> dict[str, RetrievalResult]:
    ...
    for item in results:
        ...
        sorted_docs = sorted(zip(docs, scores, strict=False), key=lambda x: x[1], reverse=True)[:top_k]
...
def merge_retrieval_results(
    results_list: list[dict[str, RetrievalResult]], output_file: str | None = None
    ...
    for query_id in merged_results:
        ...
        sorted_items = sorted(
            zip(merged_results[query_id]["docs"], merged_results[query_id]["scores"], strict=False),
            key=lambda x: x[1],
            reverse=True,
        )
