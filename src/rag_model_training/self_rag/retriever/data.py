# This code is based on the implementation from: https://github.com/AkariAsai/self-rag/tree/main/retrieval_lm/src
...
def build_mask(tensors):
    [x.shape for x in tensors]
    maxlength = max([len(x) for x in tensors])
    returnmasks = []
    ids = []
    for _k, x in enumerate(tensors):
...
def deleteword(x, p=0.1):
    ...
    x = [e for e, m in zip(x, mask, strict=False) if m > p]
    return x


def replaceword(x, min_random, max_random, p=0.1):
    mask = np.random.rand(len(x))
    x = [e if m > p else random.randint(min_random, max_random) for e, m in zip(x, mask, strict=False)]
    return x


def maskword(x, mask_id, p=0.1):
    mask = np.random.rand(len(x))
    x = [e if m > p else mask_id for e, m in zip(x, mask, strict=False)]
    return x
...
def load_passages(path):
    ...
    with open(path) as fin:
        if path.endswith(".jsonl"):
            for _k, line in enumerate(fin):
                ex = json.loads(line)
                passages.append(ex)
        else:
            reader = csv.reader(fin, delimiter="\t")
            for _k, row in enumerate(reader):
                if not row[0] == "id":
