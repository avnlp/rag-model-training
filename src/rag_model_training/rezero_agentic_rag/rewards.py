...
def reward_format(prompts: list, completions: list, **reward_kwargs) -> list:
    ...
    for completion in completions:
        ...
        if not assistant_msgs:
            rewards.append(0.0)
            for key, _ in validation_results.items():
...
def reward_em_chunk(prompts: list, completions: list, **reward_kwargs) -> list:
    ...
    if not all_supporting_paragraphs or not isinstance(all_supporting_paragraphs, list):
        ...
        msg = "supporting_paragraphs (list of lists) must be provided in reward_kwargs"
    ...
    if len(all_supporting_paragraphs) != len(completions):
        ...
        msg = "Length of supporting_paragraphs must match length of completions"
        raise ValueError(msg)

    rewards = []
    all_found_statuses = []  # Track found status for the last paragraph

    for i, (completion, required_paragraphs) in enumerate(zip(completions, all_supporting_paragraphs, strict=False)):
        if not isinstance(required_paragraphs, list):
            ...
            all_found_statuses.append(dict.fromkeys(required_paragraphs or [], False))
            continue
...
def reward_search_strategy(prompts: list, completions: list, **reward_kwargs) -> list:
    ...
    logger.info(f"Average info processing steps: {np.mean(list(validation_results['info_processing'])):.2f}")
