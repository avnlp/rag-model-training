...
from tokenizer_adapter import LlamaTokenizerAdapter, R1DistilTokenizerAdapter
    ...
    if log_file:
        with open(log_file, "a") as file:
            ...
            for i, (question, answer, student_ans, verifier_response) in enumerate(
                zip(questions, answers, student_answers, responses_text, strict=False)
            ):
                file.write(f"\n❓ Question {i + 1}:\n")
                file.write("-" * 40 + "\n")
    ...
    if output_file:
        try:
            with open(output_file, "w") as f:
                ...
                for i, (q, r, resp) in enumerate(zip(questions, rewards, final_responses, strict=False)):
                    f.write(f"\nQ{i + 1}: {q[:100]}...\n")
    ...
    if debug_file:
        try:
            ...
            for i, (q, r, resp, chat) in enumerate(
                zip(questions, rewards, final_responses, full_chat_states, strict=False)
            ):
                debug_data.append(
                    {
                        "question_id": i,
                        ...
                        "final_response": resp,
                        "chat_state": {
                            k: str(v) if isinstance(v, list | dict) else v for k, v in chat.items() if k != "tokenizer"
                        },
                    }
                )
