...
class UnslothEfficientGRPO(torch.autograd.Function):
    ...
    def forward(
        ...
        def mark_dynamic(x):
            return torch._dynamo.mark_dynamic(x, 1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if learning_rate < 1e-7:
            msg = f"Unsloth: Your learning rate of `{learning_rate}` is too small and less than 1e-7! Consider increasing it, otherwise gradient updates will be close to 0!"
            raise FloatingPointError(msg)
        if learning_rate > 1:
            msg = f"Unsloth: Your learning rate of `{learning_rate}` is way too larger > 1! Consider decreasing it to 1e-1, otherwise gradient updates will explode!"
...
class _UnslothGRPOTrainer(Trainer):
    ...
    _tag_names = ["trl", "grpo"]  # noqa: RUF012
    ...
    def __init__(
        ...
        if reward_processing_classes is None:
            ...
            msg = "The number of reward processing classes must match the number of reward functions."
            raise ValueError(msg)
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                if is_conversational(inputs[0]):
                    messages = [{"messages": p + c} for p, c in zip(prompts, completions, strict=False)]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            msg = "The GRPOTrainer does not support returning outputs"
            raise ValueError(msg)
        # Compute the per-token log probabilities for the model

        prompt_ids, _prompt_mask = inputs["prompt_ids"], inputs["prompt_mask"]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if float16 and use_bf16:
            msg = "Unsloth: Model is in float16 precision but you want to use bfloat16 precision. Set fp16 to `True` and bf16 to `False`"
            raise TypeError(msg)
        if not float16 and use_fp16:
            msg = "Unsloth: Model is in bfloat16 precision but you want to use float16 precision. Set fp16 to `False` and bf16 to `True`"
            raise TypeError(msg)
        ...
        if model is not None and hasattr(model, "for_training"):
            model.for_training()
        if "tokenizer" in locals() and hasattr(tokenizer, "padding_side"):  # noqa: F405
            tokenizer.padding_side = "right"  # noqa: F405
