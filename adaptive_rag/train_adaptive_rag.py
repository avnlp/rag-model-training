...
def prepare_scheduler(
    args, accelerator, dataloader, optimizer, max_train_steps, train_epoch
):
...
def main():
    ...
    train_dataset_for_model = train_dataset.remove_columns(
    ...
    if (
        ...
        and checkpointing_steps.isdigit()
    ):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            logger.info(
                tokenizer.batch_decode(batch["input_ids"][:1], skip_special_tokens=True)
            )
        ...
        logger.info(
            "Epoch %d Loss:{} ".format(total_loss / len(train_dataloader)), epoch
        )
