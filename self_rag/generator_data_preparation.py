...
def convert_score_to_utility_token(pred: str) -> Optional[str]:
    if not pred:
        return None
    for i in ["1", "2", "3", "4", "5"]:
        if i in pred:
...
def convert_score_to_retrieval_token(pred: str) -> Optional[str]:
    if not pred:
        return None
    if pred[0] != "[":
        pred = "[" + pred
...
def convert_score_to_groundness(pred: str) -> Optional[str]:
    if not pred:
        return None
    if pred[0] != "[":
        pred = "[" + pred
...
def postprocess_relevance_reward_token(pred: str) -> Optional[str]:
    if not pred:
        return None
    if "Relevant" in pred:
        return "[Relevant]"
...
class GeneratorDataPreparation:
    ...
    def _run_batch_evaluation(
        ...
        batch_files = [
            f for f in os.listdir(prompt_data_dir) if f.startswith("prompt_data_batch_")
        ]
        outputs: List[str] = []
    ...
    def run_retrieval_pipeline(
        self, initial_retrieval_output: str, multi_retrieval_output: str
    ) -> Dict:
        ...
        # Initial retrieval
        initial_retrieval_input = self.config["intermediate_files"][
            "initial_retrieval_input"
        ]
        ...
        initial_retrieval_result = self.config["intermediate_files"][
        ...
        multi_retrieval_input = self.config["intermediate_files"][
            "multi_retrieval_input"
        ]
        create_retrieval_input(
            input_files=[self.config["input_file"]],
        ...
        multi_retrieval_result = self.config["intermediate_files"][
            "multi_retrieval_result"
        ]
        run_retrieval(
    ...
    def prepare_isrel_issup_data(
        self, retrieval_results: Dict, multi_retrieval_output: str
    ) -> str:
        ...
        with open(retrieval_results["multi"]["result"], "r") as f:
        ...
        for batch_idx in range(num_batches):
            ...
            batch_output_file = os.path.join(
                prompt_data_dir, f"prompt_data_batch_{batch_idx:04d}.json"
            )
    ...
    def combine_all_data(
        ...
        combined_rel_output = os.path.join(
            output_dir, "intermediate", "combined_rel_preds.json"
        )
        combined_sup_output = os.path.join(
            output_dir, "intermediate", "combined_sup_preds.json"
        )
        ...
        # Load all the data
        with open(utility_output, "r") as f:
            utility_data = json.load(f)

        with open(initial_retrieval_output, "r") as f:
            retrieval_i_data = json.load(f)

        with open(multi_retrieval_output, "r") as f:
            retrieval_multi_data = json.load(f)

        with open(combined_sup_output, "r") as f:
            groundness_data = json.load(f)

        with open(combined_rel_output, "r") as f:
        ...
        for i in range(len(orig_input_data)):
            combined_item = {
                **orig_input_data[i],
                "utility_pred": (
                    utility_data[i]["pred"] if i < len(utility_data) else ""
                ),
                "retrieval_i_pred": (
                    retrieval_i_data[i]["pred"] if i < len(retrieval_i_data) else ""
                ),
                "retrieval_multi_pred": (
                    retrieval_multi_data[i]["pred"]
                    if i < len(retrieval_multi_data)
                    else ""
                ),
                "groundness_pred": (
                    groundness_data[i]["pred"] if i < len(groundness_data) else ""
                ),
                "relevance_pred": (
                    relevance_data[i]["pred"] if i < len(relevance_data) else ""
                ),
                "retrieval_evidence": (
                    retrieval_data[i].get("evidence", "")
                    if i < len(retrieval_data)
                    else ""
                ),
            }
        ...
        logging.info(
            f"Data preparation complete! Final training data saved to: {final_output}"
        )
        return final_output
    ...
    def run(self):
        ...
        retrieval_results = self.run_retrieval_pipeline(
            initial_retrieval_output, multi_retrieval_output
        )

        # Step 5: Prepare isRel and isSup input data
        prompt_data_dir = self.prepare_isrel_issup_data(
            retrieval_results, multi_retrieval_output
        )
