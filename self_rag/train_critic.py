...
class Config:
    ...
    # Data configuration
    data_path: str = field(
        default="data/critic_data.json", metadata={"help": "Path to training data"}
    )
    ...
    gradient_accumulation_steps: int = field(
        default=8, metadata={"help": "Number of steps for gradient accumulation"}
    )
    ...
    use_special_tokens: bool = field(
        default=True, metadata={"help": "Whether to add Self-RAG special tokens"}
    )
...
class CriticDataset(Dataset):
    ...
    def __init__(
        self, data_path: str, tokenizer: PreTrainedTokenizerBase, config: Config
    ):
        ...
        try:
            ...
            if not isinstance(self.data, list):
                raise ValueError("Training data should be a list of examples")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if inp:
            ...
            prompt = PROMPT_TEMPLATE.format(instruction=instr, input="").replace(
                "### Input:\n\n", ""
            )
